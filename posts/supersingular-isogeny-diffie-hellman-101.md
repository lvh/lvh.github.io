...
The j-invariant isn't particularly complicated magic; for a curve in
Weierstrass form, it's a simple function of the curve coefficients:

```
j(E) = (1728 * 4a^3)/(4a^3 + 27b^2)
```

That's a little magic, but perhaps the best analogy would be the discriminant
...
the same for the two secret curves that are generated by the DH exchange.
